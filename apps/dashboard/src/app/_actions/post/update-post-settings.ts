...
import { and, db, eq, ne, posts, projectMembers, projects, sql } from "@bloghub/db";
import { z } from "zod";
import { $getUser } from "~/app/_api/get-user";
import { zactAuthenticated } from "~/lib/zact/server";

export const updatePostSettings = zactAuthenticated(async () => {
  ...
  if (postWithSameSlug.count > 0) {
    ctx.addIssue({
      code: "custom",
      message:
        "A post with the same slug already exists",
      path: ["slug"],
    });
  }
