...
export function QRCodeCanvas(props: QRPropsCanvas) {
  const {
    ...
    foregroundColor = DEFAULT_FGCOLOR,
...
export function QRCodeSVG(props: QRPropsSVG) {
  ...
  return (
    ...
    >
      ...
      />
      <path fill={foregroundColor} d={fgPath} shapeRendering="crispEdges" />
      {image}
    </svg>
  );
}
...
export function getQRAsSVGDataUri(props: QRProps) {
  ...
  const svgData = [
    ...
    `<path fill="${backgroundColor}" d="M0,0 h${numCells}v${numCells}H0z" shapeRendering="crispEdges"></path>`,
    `<path fill="${foregroundColor}" d="${fgPath}" shapeRendering="crispEdges"></path>`,
    image,
    "</svg>",
  ].join("");
...
export async function getQRAsCanvas(
  ...
  if (calculatedImageSettings && imageSettings) {
...
export async function getQrAsString(
  props: QRProps,
  type: string,
): Promise<string> {
